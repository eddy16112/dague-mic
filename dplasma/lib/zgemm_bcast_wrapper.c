...
dague_object_t*
dplasma_zgemm_bcast_New( const int transA, const int transB,
                   ...
                   const Dague_Complex64_t beta,  tiled_matrix_desc_t* C)
{
    ...
    if( PlasmaNoTrans == transA ) {
        if( PlasmaNoTrans == transB ) {
            dague_zgemm_NN_bcast_object_t* object;
            object = dague_zgemm_NN_bcast_new(transA, transB, alpha, beta,
                                        *A, (dague_ddesc_t*)A,
                                        *B, (dague_ddesc_t*)B,
                                        *C, (dague_ddesc_t*)C);
            arena = object->arenas[DAGUE_zgemm_NN_bcast_DEFAULT_ARENA];
...
void
dplasma_zgemm_bcast_Destruct( dague_object_t *o )
{
    int transA = ((dague_zgemm_NN_bcast_object_t *)o)->transA;
    int transB = ((dague_zgemm_NN_bcast_object_t *)o)->transB;

    dplasma_datatype_undefine_type( &(((dague_zgemm_NN_bcast_object_t *)o)->arenas[DAGUE_zgemm_NN_bcast_DEFAULT_ARENA]->opaque_dtt) );

    if( PlasmaNoTrans == transA ) {
        if( PlasmaNoTrans == transB ) {
            dague_zgemm_NN_bcast_destroy((dague_zgemm_NN_bcast_object_t *)o);
        } /*else {
...
void
dplasma_zgemm_bcast( dague_context_t *dague, const int transA, const int transB,
               const Dague_Complex64_t alpha, const tiled_matrix_desc_t *A, const tiled_matrix_desc_t *B,
               const Dague_Complex64_t beta,  tiled_matrix_desc_t *C)
{
    dague_object_t *dague_zgemm = NULL;

    dague_zgemm = dplasma_zgemm_bcast_New(transA, transB,
                                    alpha, A, B,
                                    beta, C);
    ...
    if ( dague_zgemm != NULL )
    {
        ...
        dplasma_zgemm_bcast_Destruct( dague_zgemm );
    }
}
