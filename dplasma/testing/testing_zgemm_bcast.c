...
int main(int argc, char ** argv)
{
    ...
    if(!check)
    {
        ...
        /* Create DAGuE */
        PASTE_CODE_ENQUEUE_KERNEL(dague, zgemm_bcast,
                                           (tA, tB, alpha,
        ...
        /* lets rock! */
        PASTE_CODE_PROGRESS_KERNEL(dague, zgemm_bcast);

        dplasma_zgemm_bcast_Destruct( DAGUE_zgemm_bcast );
...
/* Iterate on the transpose forms. TODO: LDB is set incorrecly for T and H */
        ...
        for(tA=0; tA<2; tA++) {
            for(tB=0; tB<2; tB++) {
                ...
                dplasma_zgemm_bcast(dague, trans[tA], trans[tB],
