...
/*
  ...
  COMMSTEP_INIT      - Signal the return of communication thread in initial state
  ...
  COMMSTEP_UPDOEND   - Signal the end of different steps during backward/forward substitution
...
/*
 * Revert the steal algorithm in dynamic scheduler
 * USELESS
 */
/* #define PASTIX_REVERTSTEAL */
 ...
 * inconnus
 */
#if (defined PASTIX_DYNSCHED) && (!(defined THREAD_COMM)) && !(defined FORCE_NOMPI)
#define THREAD_COMM
#endif
...
/* Ne peut pas fonctionner a cause de la répartition des taches dans les superieurs */
...
#if (defined SMP_SOPALIN) && (!(defined THREAD_COMM)) && (!(defined EXACT_TAG)) && (!(defined EXACT_THREAD))
#define EXACT_THREAD
#endif
...
#ifdef CHOL_SOPALIN
#ifdef SOPALIN_LU
#define API_CALL(nom) PASTIX_PREFIX_F(U ## nom)
#else
#define API_CALL(nom) PASTIX_PREFIX_F(L ## nom)
#endif
#else
#define API_CALL(nom) PASTIX_PREFIX_F(D ## nom)
#endif
