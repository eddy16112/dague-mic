...
dsparse_zpotrf_sp_New(sparse_matrix_desc_t *A)
{
    ...
    dague_zpotrf_sp = dague_zpotrf_sp1dplus_new(A, (dague_ddesc_t *)A, NULL, NULL );

    dague_zpotrf_sp->p_work1 = (dague_memory_pool_t*)malloc(sizeof(dague_memory_pool_t));
    dague_private_memory_init( dague_zpotrf_sp->p_work1, (A->pastix_data->solvmatr).coefmax * sizeof(Dague_Complex64_t) );

    dague_zpotrf_sp->p_work2 = (dague_memory_pool_t*)malloc(sizeof(dague_memory_pool_t));
    dague_private_memory_init( dague_zpotrf_sp->p_work2, (A->pastix_data->solvmatr).coefmax * sizeof(Dague_Complex64_t) );
...
dsparse_zpotrf_sp_Destruct( dague_object_t *o )
{
    ...
    dague_private_memory_fini( dague_zpotrf_sp->p_work1 );
    free( dague_zpotrf_sp->p_work1 );
    dague_private_memory_fini( dague_zpotrf_sp->p_work2 );
    free( dague_zpotrf_sp->p_work2 );
