...
int sparse_sgemm_cuda_fini(dague_context_t* dague_context)
{
    ...
    sparse_gpu_data_map_fini(&gpu_data);
                         ...
                         CUstream stream )
{
    ...
    on_gpu = sparse_gpu_data_is_on_gpu(gpu_device, gpu_data_C,
                           ...
                           CUstream stream )
{
    ...
    DEBUG2(("Request GPU runs SPARSE_GEMM(%d, %d, %d)\n",
    ...
    free(myA); free(myCgpu); free(myCcpu);
#endif
...
#if defined(DSPARSE_GPU_WITH_SYNCHRO)
        status = cuStreamSynchronize( stream );
        DAGUE_CUDA_CHECK_ERROR( "cuStreamSynchronize", status,
                      ...
                      dague_execution_context_t* this_task )
{
    ...
    which_gpu = sparse_gpu_data_cblk_write_owner( &gpu_data, fcblk );
    if( which_gpu < 0 ) {
        /* this is the first time we see this tile.
           Let's decide which GPU will work on it. */
        which_gpu = 0; /* TODO */
#if DPLASMA_SCHEDULING
        assert( fcblk < SYMB_CBLKNBR );
        if(ndevices > 1){
            /* reverse odd-even */
            /* homogeneous GPU */
            {
        ...
        **/
        ...
        gpu_load[which_gpu] += fcblk;  /* keep n -- not being used yet*/
    ...
    DEBUG2(( "Add sparse_gemm(blok = %d, cblk = %d, fcblk = %d) priority %d\n",
             this_task->locals[0].value,
...
/*    printf("k=%d, m=%d, n=%d\n",k,m,n);*/
    ...
    gpu_load[gpu_device->index]+=fcblk;
#endif
 ...
 more_work_to_do:
    if( (NULL != this_task) && (NULL == progress_array[submit]) ) {
        ...
        rc = gpu_sgemm_internal( gpu_device, eu_context, this_task, gpu_device->streams[submit] );
        if( 0 != rc ) {  /* something fishy happened. Reschedule the pending tasks on the cores */
            goto disable_gpu;
        }
        DEBUG3(( "GPU:\tsubmit %p (k = %d, m = %d, n = %d) [%d]\n", (void*)progress_array[submit], blok, fcblk, cblk, submit ));
 ...
 complete_previous_work:
    /* Everything went fine so far, the result is correct and back in the main memory */
    DEBUG3(( "GPU:\tcomplete %p (k = %d, m = %d, n = %d) [%d]\n", (void*)progress_array[waiting], blok, fcblk, cblk, waiting ));
                              ...
                              dague_gpu_data_map_t* gpu_map )
{
    if( NULL != gpu_map->data_map ) {
        ...
        gpu_map->data_map = (memory_elem_t**)calloc( SYMB_CBLKNBR, sizeof(memory_elem_t*));
    }
...
int sparse_gpu_mark_data_usage( dague_gpu_data_map_t* gpu_map,
                                int type, int cblk )
{
    memory_elem_t* this_elem;
    
    if( (NULL == gpu_map) || (NULL == gpu_map->data_map) ||
        (NULL == (this_elem = gpu_map->data_map[cblk])) ) {
                              ...
                              memory_elem_t **pmem_elem )
{
    ...
    where_from = &(gpu_map->data_map[cblk]);
                               ...
                               gpu_elem_t **pgpu_elem)
{
    ...
    sparse_gpu_data_get_cblk( gpu_map, cblk, &this_elem );
