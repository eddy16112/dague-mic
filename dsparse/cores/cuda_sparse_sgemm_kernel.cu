...
GENERATE_SM_VERSION_NAME(sparse_sgemm_kernel_N_T_64_16_4_16_4)(float *C, const float *A, const float *B,
				     int m, int n, int k, 
				     int lda, int ldb, int ldc, 
				     float alpha, float beta, 
				     int blocnbr, const int * bloc_idx, const int * facing_bloc_idx) 
{
  /*  -- MAGMA (version 1.1) --
      Univ. of Tennessee, Knoxville
      Univ. of California, Berkeley
  ...
  */
     ...
     
  {
    ...
    int blocnum = 0;
    int idx_x = ibx +idt;
    int bloc_start = bloc_idx[blocnum]-bloc_idx[0];
    if (blocnbr > 1) {
      int bloc_end = bloc_idx[blocnum+1]-bloc_idx[0];
      while ( blocnum < blocnbr && 
	      ( ! (idx_x >= bloc_start && idx_x < bloc_end) ))
	{
	  blocnum ++;
	  bloc_start = bloc_end;
	  if (blocnum+1 < blocnbr)
	    bloc_end = bloc_idx[blocnum+1]-bloc_idx[0];
	  else
	    bloc_end = m;
	}
    }
    if (blocnum < blocnbr) {
      stride = facing_bloc_idx[blocnum] - bloc_start;
    }
    
    C += idx_x + stride + __mul24(iby,ldc);
  }
...
/* 	  C, A, B, m, n ,k, lda, ldb, ldc, alpha, beta, blocnbr, sizeof(float), sizeof(int)); */
  GENERATE_SM_VERSION_NAME(sparse_sgemm_kernel_N_T_64_16_4_16_4)<<< grid, threads  /* , 0, starpu_cuda_get_local_stream()  */ >>>(C, A, B,	
	 											 m, n, k, 
												 lda, ldb, ldc,
												 alpha,  beta, 
												 blocnbr, 
